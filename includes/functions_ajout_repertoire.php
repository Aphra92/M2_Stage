	...
	foreach ($tab as $newrep) {
		...
		if (file_exists($filepath)) {
			...
			if (file_exists($fileruninf)) {
				...
				$namenoqc = explode('_QC',$name);
				$fileruninf = $fileruninf.'/'.$namenoqc[0].'/';
				if (file_exists($fileruninf)) {
					//echo $fileruninf;
...
function afterrepertoire($tab, $pdo) {
	...
	foreach ($tab as $newrep) {
		...
		if ($next) {
			...
			///////////////////////////////////////////
			///////////////////////////////////////////
			// Si $m > 1 on actualise les count pour tous les tcr trouvé a l'analyse précédentes
			if($nana > 1) {
				$i = $nana - 1;
				remove_tcr($p,$e,$o,$s,$a,$m,$runid,$repid,$i, $pdo);
			}
			///////////////////////////////////////////
			///////////////////////////////////////////
			///////////////////////////////////////////
...
function remove_tcr($p,$e,$o,$s,$a,$m,$runid,$repid,$i, $pdo) {
	// On selectionne tout les tcr_id associé au count de la table tcr
	$query = "SELECT tcr.tcr_id,  tcr.tcr_count 
	FROM tcr.tcr_obs, tcr.tcr 
	WHERE p_id = :p 
	AND e_id = :e
	AND o_id = :o
	AND s_id = :s
	...
	$request = $pdo->prepare($query);
	$pdo->beginTransaction();
	// On recupere tout les tcr_obs pour le repertoire et la valeur de count associer au tcr
	$request -> bindValue(':p',$p, PDO::PARAM_STR);
	$request -> bindValue(':e',$e, PDO::PARAM_STR);
	$request -> bindValue(':o',$o, PDO::PARAM_STR);
	$request -> bindValue(':s',$s, PDO::PARAM_STR);
	$request -> bindValue(':a',$a, PDO::PARAM_STR);
	$request -> bindValue(':m',$m, PDO::PARAM_STR);
	$request -> bindValue(':runid',$runid, PDO::PARAM_STR);
	$request -> bindValue(':repid',$repid, PDO::PARAM_STR);
	$request -> bindValue(':i',$i, PDO::PARAM_STR);
	$request -> execute();
	$pdo->commit();
	...
	while($donnees = $request->fetch(PDO::FETCH_ASSOC)) {
		$tcrid[$donnees['tcr_id']] = $donnees['tcr_count'];
	}
	//~ p($tcrid);
	// on parcour le tableau des tcr_id
	$tcrtodelete = array();
	foreach(array_keys($tcrid) as $k) {
		// pour tous les cas, on diminue le count de 1 (des tcr peuvent se retrouver a 0 mais ils sont conservés)
		
		// On update le tcr avec count = count - 1
		$pdo->beginTransaction();
		$newcount = $tcrid[$k]-1;
		$query = "UPDATE tcr.tcr SET tcr_count = '".$newcount."' WHERE tcr_id = '".$k."';";
		$request = $pdo->prepare($query);
		$request -> execute();
		$pdo->commit();
		
		// On garde les tcr_obs
		// On supprime le tcr_obs
		//~ $pdo->beginTransaction();
		//~ $query = "DELETE FROM tcr.tcr_obs WHERE p_id = :p 
		//~ AND e_id = :e
		//~ AND o_id = :o
		//~ AND s_id = :s
		//~ AND a_id = :a
		...
		//~ $request -> bindValue(':repid',$repid, PDO::PARAM_STR);
		//~ $request -> bindValue(':i',$i, PDO::PARAM_STR);
		//~ $request -> bindValue(':tcr',$k, PDO::PARAM_STR);
		//~ $request -> execute();
		//~ $pdo->commit();
	}
}
